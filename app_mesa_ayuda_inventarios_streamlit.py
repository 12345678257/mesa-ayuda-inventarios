...
def page_cmdb():
    ...
    if not cis.empty:
        ...
        st.info("Crea primero al menos 1 CI para registrar relaciones.")
...
def page_tickets_nuevo():
    ...
    if submitted:
        ...
        _dfcnt = run_query("SELECT COUNT(*) n FROM tickets WHERE substr(created_at,1,10)=?", (today_str,))
        _seq = int(_dfcnt.loc[0,'n']) + 1
        code = f"TCK-{datetime.utcnow().strftime('%Y%m%d')}-{_seq:04d}"
        ...
        allowed_cats = ['Incidente','Solicitud','Ajuste','Consulta']
        category_value = itil_type if itil_type in allowed_cats else 'Consulta'
        run_script("""INSERT INTO tickets(code,title,description,category,priority,urgency,impact,status,sla_hours,response_sla_hours,created_by,assigned_to,
                      ...
                      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                   (code, title.strip(), desc, category_value, prio_value, urgency, impact, "Abierto", int(resolution_sla), int(response_sla), require_login()['id'], None,
