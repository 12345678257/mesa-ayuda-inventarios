...
def page_cmdb():
    ...
    if not cis.empty:
        with st.form("ci_rel_form"):
            p = st.selectbox("CI Padre", cis['name']); c = st.selectbox("CI Hijo", cis['name']); r = st.text_input("Tipo de relación (depende de, usa, replica, etc.)")
            submitted2 = st.form_submit_button("Crear relación")
        if submitted2:
            pid = int(cis.loc[cis['name']==p,'id'].iloc[0]); cid = int(cis.loc[cis['name']==c,'id'].iloc[0])
            if pid == cid: st.error("Padre y Hijo no pueden ser el mismo."); 
            else: run_script("INSERT INTO ci_relations(parent_ci_id,child_ci_id,relation_type) VALUES(?,?,?)", (pid,cid,r.strip())); st.success("Relación creada.")
