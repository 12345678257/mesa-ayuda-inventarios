...
import os, sqlite3, smtplib, ssl, random, string, hmac, hashlib as _hashlib, json, requests, io
...
def send_email(to_email: str, subject: str, body: str):
    # Prioriza settings; cae a env vars si no están
...
def try_token_sso():
    try:
        ...
        if df.empty:
            run_script("INSERT INTO users(username,password,role,created_at) VALUES(?,?,?,datetime('now'))", (user, hash_password("Temporal123!"), "usuario"))
            df = run_query("SELECT * FROM users WHERE username=?", (user,))
...
def page_login():
    ...
    with tab_login:
        ...
        if st.button("Entrar", key="btn_login", help="Iniciar sesión"):
            if login(u,p):
...
def page_tickets_nuevo():
    ...
    if st.button("Crear", key="btn_create_ticket", help="Crear ticket en la mesa de ayuda", type="primary"):
        ...
        run_script("""INSERT INTO tickets(code,title,description,category,itil_type,service_id,priority,urgency,impact,status,created_by,created_at,updated_at,response_due_at,due_at)
                      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                   (code_t,title.strip(),desc.strip(),category,itil_type,int(service_id),priority,urgency,impact,"Nuevo",int(user["id"]),now,now,response_due,resolve_due))
        # Guardar adjuntos
        if files:
            up = get_upload_root()
            # Obtener ID del ticket recien creado
            tid = int(run_query("SELECT id FROM tickets WHERE code=?", (code_t,)).loc[0,"id"])
            for f in files:
                safe = f.name.replace("..","_").replace("/","_").replace("\\","_")
                folder = os.path.join(up, "ticket_attachments")
                os.makedirs(folder, exist_ok=True)
                dest = os.path.join(folder, f"{code_t}_{safe}")
...
def page_tickets_bandeja():
    ...
    def _grid(itil):
        ...
        if team_filter and team_filter!="Todos":
            # por equipo del agente asignado
...
def page_ticket_detalle():
    ...
    t = run_query("""SELECT t.*, s.name as service_name, u.username as owner_name, u.email as owner_email, ag.username as agent_name, ag.email as agent_email
                     FROM tickets t
                     LEFT JOIN services s ON s.id=t.service_id
                     JOIN users u ON u.id=t.created_by
                     LEFT JOIN users ag ON ag.id=t.assigned_to
    ...
    if user["role"] in ("admin","agente"):
        ...
        if c1.button("Asignar", key="btn_assign", help="Asignar agente"):
        ...
        if c2.button("Aplicar estado", key="btn_state", help="Cambiar estado"):
    ...
    # Workflow de Aprobaciones para Cambios
    if row["itil_type"]=="Cambio":
        ...
        if flow.empty:
            ...
            if pending.empty:
                # Sembrar instancias de aprobación
                for i, r in flow.iterrows():
                    run_script("INSERT INTO change_approvals(ticket_id,service_id,area_id,level,status) VALUES(?,?,?,?,?)", (int(tid), int(row["service_id"]), int(run_query("SELECT id FROM areas WHERE name=?", (r["area"],)).loc[0,"id"]), int(r["level"]), "Pendiente"))
            ...
            # Acción de aprobar/rechazar para el área del usuario
            if user["role"] in ("admin","agente"):
                # Asumimos que agentes pueden aprobar si pertenecen al área? Simplificación: aprobador manual
                ...
                if st.button("Registrar decisión", key="btn_appr"):
                    ...
                    # Si todos aprobados → pasar a 'Aprobación' o 'En Progreso'
    ...
    if upfiles:
        ...
        for f in upfiles:
            safe = f.name.replace("..","_").replace("/","_").replace("\\","_")
            folder = os.path.join(up, "ticket_attachments"); os.makedirs(folder, exist_ok=True)
            dest = os.path.join(folder, f"{row['code']}_{safe}")
...
def page_activos():
    ...
    if not a.empty:
        ...
        with t4:
            # Adjuntos generales del activo
            st.subheader("Adjuntos del activo")
            af = run_query("SELECT id, file_name, file_path, uploaded_at, file_type FROM asset_files WHERE asset_id=? ORDER BY uploaded_at DESC", (int(r["id"]),))
            if not af.empty:
                for i, r0 in af.iterrows():
                    try:
                        with open(r0["file_path"], "rb") as fh:
            ...
            new_af = st.file_uploader("Subir adjuntos", accept_multiple_files=True, key="af_upload")
...
def page_configuracion():
    ...
    with tabs[1]:
        ...
        if st.button("Crear servicio", key="btn_svc_create"): run_script("INSERT INTO services(name) VALUES(?)", (new_s,)); st.rerun()
    ...
    with tabs[4]:
        ...
        st.subheader("SMTP (persistente en DB, solo admin)")
        ...
        st.write("- **Webhooks**: define SLACK_WEBHOOK_URL / TEAMS_WEBHOOK_URL / DISCORD_WEBHOOK_URL")
...
# Llamar esto desde migrate_schema
