...
def page_tickets_nuevo():
    ...
    if st.button("Crear", key="btn_create_ticket", help="Crear ticket en la mesa de ayuda", type="primary"):
        ...
        run_script("""INSERT INTO tickets(code,title,description,category,itil_type,service_id,priority,urgency,impact,status,created_by,created_at,updated_at,response_due_at,due_at)
                      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                   (code,title.strip(),desc.strip(),category,itil_type,int(service_id),priority,urgency,impact,"Nuevo",int(user["id"]),now,now,response_due,resolve_due))
        notify_webhooks("ticket_created", {"code": code, "title": title, "type": itil_type, "priority": priority})
...
def page_configuracion():
    ...
    with tabs[4]:
        ...
        st.write("- **SMTP**: configura variables de entorno (SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD).")
