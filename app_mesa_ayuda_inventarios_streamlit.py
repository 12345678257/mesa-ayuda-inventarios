...
def page_ticket_detalle():
    ...
    t = run_query("""SELECT t.*, s.name as service_name, u.username as owner_name, u.email as owner_email, ag.username as agent_name
                     FROM tickets t
                     LEFT JOIN services s ON s.id=t.service_id
                     JOIN users u ON u.id=t.created_by
                     LEFT JOIN users ag ON ag.id=t.assigned_to
    ...
    if user["role"] in ("admin","agente"):
        ...
        if c2.button("Aplicar estado", key="btn_state", help="Cambiar estado"):
            ...
            # Encuesta al cerrar
            if new_status=="Cerrado" and row["owner_email"]:
